...
class StructureTestSuite(unittest.TestCase):
    ...
    def test_class_exists_currencyexchangeerror(self):
        classes = _get_class_names(self.MODULE)
        self.assertIn(
            base64.b64decode(b"Q3VycmVuY3lFeGNoYW5nZUVycm9y").decode(),
            classes,
            msg=f"The class "
            f"`{base64.b64decode(b'Q3VycmVuY3lFeGNoYW5nZUVycm9y').decode()}` "
            f"is not found, but it was marked as required.",
        )

    def test_class_exists_exchangerateunknownerror(self):
        classes = _get_class_names(self.MODULE)
        self.assertIn(
            base64.b64decode(b"RXhjaGFuZ2VSYXRlVW5rbm93bkVycm9y").decode(),
            classes,
            msg=f"The class "
            f"`{base64.b64decode(b'RXhjaGFuZ2VSYXRlVW5rbm93bkVycm9y').decode()}` "
            f"is not found, but it was marked as required.",
        )

    def test_class_exists_invalidcurrencyerror(self):
        classes = _get_class_names(self.MODULE)
        self.assertIn(
            base64.b64decode(b"SW52YWxpZEN1cnJlbmN5RXJyb3I=").decode(),
            classes,
            msg=f"The class "
            f"`{base64.b64decode(b'SW52YWxpZEN1cnJlbmN5RXJyb3I=').decode()}` "
            f"is not found, but it was marked as required.",
        )
